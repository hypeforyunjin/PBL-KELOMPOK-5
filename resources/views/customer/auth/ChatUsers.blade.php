<!-- filepath: resources/views/customer/auth/Chat.blade.php -->
<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8" />
...
<body>
    <div class="flex flex-col min-h-screen bg-white justify-between">
        <div class="flex items-center bg-white p-4 pb-2 justify-between">
            <h2 class="text-[#111518] text-lg font-bold flex-1 text-center">Chat dengan Admin</h2>
        </div>
        <div id="chat-messages" class="flex-1 overflow-y-auto px-4 py-4 bg-white space-y-4"></div>
        <form id="chat-form" action="{{ route('chat.store') }}" method="POST" class="flex items-center px-4 py-3 gap-3 border-t bg-white">
            @csrf
            <input type="text" name="message" id="message-input" placeholder="Tulis pesan ke admin..."
